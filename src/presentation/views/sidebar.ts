...
import type { AppState } from '../../services/app-service'
...
export function renderSidebar(state: AppState, service: AppService): TemplateResult {
  return html`
    <aside class="fixed lg:static inset-y-0 left-0 z-40 w-[250px] h-full bg-[var(--color-things-sidebar)] flex flex-col overflow-y-auto select-none border-r border-[var(--color-things-divider)] transform transition-transform duration-200 ${state.sidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0">
      <div class="flex items-center justify-between px-3 pt-3 pb-1 lg:hidden">
        ...
        <button class="w-6 h-6 flex items-center justify-center text-[var(--color-things-secondary)] hover:text-[var(--color-things-text)]" @click=${() => service.closeSidebar()}>
          <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
        </button>
      </div>
      <nav class="flex-1 px-3 pt-2 lg:pt-3 pb-2">
        <ul class="space-y-0">
          ${NAV_ITEMS.map(item => navItem(item, state.currentPath))}
        </ul>
        <ul class="mt-1 space-y-0">
          ${navItem(LOGBOOK_ITEM, state.currentPath)}
        </ul>
        <div class="mt-8 mb-2 px-2 flex items-center justify-between">
          ...
          <button class="w-5 h-5 flex items-center justify-center text-[var(--color-things-muted)] hover:text-[var(--color-things-text)] rounded transition-colors"
            @click=${async () => {
              const { showProjectDialog } = await import('./project-dialog')
              const result = await showProjectDialog()
              if (result) service.createProject(result.name, result.color)
            }}>
            <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15" /></svg>
          </button>
        </div>
        <ul class="space-y-0">
          ${state.projects.map(p => projectItem(p, state.currentPath))}
        </ul>
        <div class="mt-8 mb-2 px-2 flex items-center justify-between">
          <span class="text-[11px] font-semibold text-[var(--color-things-secondary)] uppercase tracking-wide">Tags</span>
          <button class="w-5 h-5 flex items-center justify-center text-[var(--color-things-muted)] hover:text-[var(--color-things-text)] rounded transition-colors"
            @click=${async () => {
              const { showTagDialog } = await import('./tag-dialog')
              const result = await showTagDialog()
              if (result) service.createTag(result.name, result.color)
            }}>
            <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15" /></svg>
          </button>
        </div>
        <ul class="space-y-0">
          ${state.tags.map(t => tagItem(t, state.currentPath))}
        </ul>
      </nav>
...
function projectItem(p: ProjectData, currentPath: string): TemplateResult {
...
function tagItem(t: TagData, currentPath: string): TemplateResult {
  const isActive = currentPath === `/tags/${t.id}`
  return html`
    <li>
...
function getColorVar(color: string): string {
  const map: Record<string, string> = {
    ...
    purple: '#AF52DE', pink: '#FF2D55', gray: '#8E8E93', brown: '#A2845E',
  }
